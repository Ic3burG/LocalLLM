...
async def test_react_loop_sse_with_deep_think():
    ...
    messages = [{"role": "system", "content": "You are an assistant."}, {"role": "user", "content": "What is 2+2?"}]
    model_id = "test-model"
    ...
    with patch("agent.run_deep_thinking_pipeline", new_callable=AsyncMock) as mock_dt, \
        ...
        assert any("<thought>\n2+2 is 4 because of math.\n</thought>" in m["content"] for m in messages if m["role"] == "user")
